...
for checkpoint_path in checkpoint_paths:
    ...
    for validation_path in os.listdir(args.validation_path):
        ...
        prompt = (
            Image.open(os.path.join(validation_path, "1.jpg"))
            .convert("RGB")
            .resize((args.resolution, args.resolution))
        )
        blueprint = (
            Image.open(os.path.join(validation_path, "2.jpg"))
            .convert("RGB")
            .resize((args.resolution, args.resolution))
        )
        blueprint = Image.eval(blueprint, lambda x: 255 - x)
        ...
        image.save(
            os.path.join(
                args.output_dir, f"{checkpoint}-{os.path.basename(validation_path)}.jpg"
            )
        )
