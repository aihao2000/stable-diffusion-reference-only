...
def log_validation(
    ...
    pipeline: StableDiffusionReferenceOnlyPipeline = StableDiffusionReferenceOnlyPipeline.from_pretrained(
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                tracker.writer.add_images(prompt_path, formatted_images, step, dataformats="NHWC")
...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Simple example of a UNet2DDobuleConditionModel training script.")
    parser.add_argument(
    ...
    if args.validation_prompt is not None and args.validation_blueprint is None:
        raise ValueError("`--validation_blueprint` must be set if `--validation_prompt` is set")

    if args.validation_prompt is None and args.validation_blueprint is not None:
        raise ValueError("`--validation_prompt` must be set if `--validation_blueprint` is set")
...
def make_train_dataset(args, clip_image_processor, accelerator):
    ...
    def preprocess_train(examples):
        ...
        blueprints = [cv2.cvtColor(np.array(blueprint), cv2.COLOR_RGB2GRAY) for blueprint in blueprints]
        blueprints = [
        ...
        blueprints = [Image.fromarray(blueprint).convert("RGB") for blueprint in blueprints]
        blueprints = [Image.eval(blueprint, lambda x: 255 - x) for blueprint in blueprints]
        blueprints = [blueprint_transforms(blueprint) for blueprint in blueprints]
        prompts = [
            clip_image_processor(prompt, return_tensors="pt").pixel_values[0] for prompt in examples[prompt_column]
        ]
    ...
    with accelerator.main_process_first():
        ...
        if args.load_dataset_streaming:
            ...
            if args.dataset_map:
                train_dataset = dataset["train"].map(preprocess_train, batch_size=8, batched=True)
            else:
...
def collate_fn(examples):
    ...
    blueprint_pixel_values = torch.stack([example["blueprint_pixel_values"] for example in examples])
    blueprint_pixel_values = blueprint_pixel_values.to(memory_format=torch.contiguous_format).float()

    prompt_pixel_values = torch.stack([example["prompt_pixel_values"] for example in examples])
...
def main(args):
    ...
    def save_model_hook(models, weights, output_dir):
        for model in models:
            sub_dir = "unet" if isinstance(model, type(accelerator.unwrap_model(unet))) else "image_encoder"
    ...
    def load_model_hook(models, input_dir):
        while len(models) > 0:
            ...
            if isinstance(model, type(accelerator.unwrap_model(image_encoder))):
                ...
                load_model = CLIPVisionModel.from_pretrained(input_dir, subfolder="image_encoder")
                model.config = load_model.config
            else:
                # load diffusers style into model
                load_model = UNet2DDobuleConditionModel.from_pretrained(input_dir, subfolder="unet")
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            resume_progress_bar = tqdm(range(0, resume_step), disable=not accelerator.is_local_main_process)
