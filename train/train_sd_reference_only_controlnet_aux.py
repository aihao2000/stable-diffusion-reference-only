...
def log_validation(
    ...
    for i, log in enumerate(image_logs):
        for j, image in enumerate(log["images"]):
            image.save(
                os.path.join(args.output_dir, "validations", f"{step}_{i}_{j}.jpg")
            )
...
def save_model_card(repo_id: str, image_logs=None, base_model=str, repo_folder=None):
    ...
    if image_logs is not None:
        ...
        for i, log in enumerate(image_logs):
            ...
            validation_blueprint.save(os.path.join(repo_folder, "blueprint.jpg"))
            img_str += f"prompt: {validation_prompt}\n"
            images = [validation_blueprint] + images
            image_grid(images, 1, len(images)).save(
                os.path.join(repo_folder, f"images_{i}.jpg")
            )
            img_str += f"![images_{i})](./images_{i}.jpg)\n"
