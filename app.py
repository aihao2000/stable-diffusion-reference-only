...
segment_model = get_anime_segmentation_model(
    model_path=hf_hub_download("skytnt/anime-seg", "isnetis.ckpt")
).to(device)
...
with gr.Blocks(delete_cache=(3600, 3600)) as demo:
    ...
    - Load examples and click ```inference``` to try it out.\n\n
    - If the uploaded character reference image contains a background, you can click on the chacacter segment to clear the background for better results.
    - Blueprint recommends using a line art with a black background and white lines. You can click ```get line art``` to get the line art with a white background and black lines from the character image, and then click ```color inversion``` to get it.
    - If you want to change the image size, input the new size and click ```resize```
    - ```automatic coloring``` automatically converts line drawings with black lines on a white background to white lines on a black background, and then ```inference```.
    - ```style transfer``` automatically performs ```anime segment```, ```get line art```, ```color inversion``` for the uploaded image. Then ```inference```.
    """
    )
    with gr.Row():
        with gr.Column():
    ...
    with gr.Row():
        gr.Examples(
            examples=[
                [
                    os.path.join(
                        os.path.dirname(__file__), "README.assets", "3x9_prompt.jpg"
                    ),
                    os.path.join(
                        os.path.dirname(__file__), "README.assets", "3x9_blueprint.jpg"
                    ),
                ],
            ],
